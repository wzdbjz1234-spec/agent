...
- 密码、API token、私钥、Cookie、连接串命中即阻断；常见 PII 使用当前 Agent scope 内稳定、类型化占位。
- 占位只改变云端视图，不修改 Workspace/Dataset；工具参数进入 Sandbox 前只恢复已登记且类型匹配的占位。
- 映射保存在 `runtime-data/privacy/{scope_id}.db`，不得进入 Runtime DB、Workspace、Sandbox、日志、trace 或 Artifact。
