...
|---:|---|---|---|---|---|
| 00 | Project foundation | packaging, config, CI | — | NOT_STARTED | — |
| 01 | Domain model and state machines | domain | 00 | NOT_STARTED | — |
| 02 | Runtime storage and durable primitives | storage | 01 | NOT_STARTED | — |
| 03 | Workspace, import and publication | workspace, providers/workspace | 01–02 | NOT_STARTED | — |
...
- 建立配置模型：runtime-data 根、Workspace 根、Privacy 根、模型 Provider、OpenSandbox endpoint、预算与资源限制。
...
- 实现 Session、Task、Run、AnalysisStep、Dataset、Artifact、Finding、Lineage 和领域错误。
...
- 实现 Task/Run/Step、Dataset/Artifact/Finding/Lineage、event、checkpoint metadata repository。
...
### Phase 03 — Workspace, import and publication

Objective：实现不可变输入、受控写入和可崩溃恢复的正式输出发布。

Deliverables:

- 实现 VirtualWorkspace/WorkspaceBridge 和 LocalWorkspace Adapter。
- 原子创建 `inputs/working/staging/datasets/artifacts/state`。
- 实现 CSV、Parquet、Excel、JSON 及可选 DuckDB/SQLite snapshot 导入。
- 实现格式嗅探、规范化文件名、大小/类型/hash 校验和输入只读策略。
- 实现路径规范化、真实路径检查、符号链接/设备文件/目录穿越拒绝。
- 实现 STAGED -> publish -> AVAILABLE 协议、幂等键与 reconciler。

Exit Gate:

- Agent/Sandbox 无法覆盖或删除 inputs。
- `..`、Host 绝对路径、符号链接和跨 Task 引用均被拒绝。
- 在发布各断点注入崩溃后，reconciler 能收敛到 AVAILABLE、可重试或明确损坏状态。
- API/上层只能看到 AVAILABLE 输出，正式对象有稳定 ID 和 hash。

Required report: `doc/phase-03-workspace-YYYYMMDD.md`
...
- PII 到达 fake cloud 时只有占位；Workspace 原始文件和 hash 不变。
...
- 实现 `list_workspace`、`read_text`、`inspect_output` 和 `submit_finding` 的窄 Interface。
...
- 每个 FindingCandidate 可追溯到 Step、输入、代码或 Artifact。
...
- 实现 PydanticAI checkpoint metadata、Sandbox lease 关联和恢复决策。
...
- 实现 checkpoint/compaction：持久化目标、计划、进度、领域引用和未解决问题。
...
- 实现 FastAPI Task 创建/查询/取消/恢复/重试、事件、Dataset、Artifact 和受控文件接口。
...
- 每个 VERIFIED Finding 至少有一条 hash 未变且属于当前 Task/Run 的有效证据链。
...
- 建立真实本地 API + SQLite + Workspace + OpenSandbox + fake cloud model 的 E2E harness。
- 覆盖 CSV/Parquet/Excel/JSON 导入、隐私出口、SQL/Python、发布、lineage、Finding 和回答。
...
|---|---|---|
| Unit | 纯领域规则、检测规则、路径、hash、预算、错误分类 | 网络、真实 OpenSandbox、真实模型 |
| Contract | Sandbox/Workspace/ModelGateway/Durable Interface 的共享行为 | 断言具体 SDK 内部结构 |
| Integration | SQLite、文件系统、OpenSandbox、PydanticAI 与 Adapter 组合 | 访问真实云账号或生产数据 |
